...
class ReadTests {
    ...
    companion object {
        ...
        @JvmStatic
        private fun getResource(name: String): BufferedSource {
            val resource = checkNotNull(Thread.currentThread().contextClassLoader.getResourceAsStream(name)) {
                "Could not find resource $name!"
            }
            return resource.source().buffer()
        }

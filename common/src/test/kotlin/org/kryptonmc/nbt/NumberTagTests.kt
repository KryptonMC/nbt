...
class NumberTagTests {
    ...
    fun `check number conversions`() {
        ...
        assertEquals(double.toShort(), (double.value.toInt() and 0xFFFF).toShort())
        assertEquals(double.toByte(), (double.value.toInt() and 0xFF).toByte())
        val float = FloatTag.of(83.7F)
        assertEquals(float.toInt(), float.value.toInt())
        assertEquals(float.toShort(), (float.value.toInt() and 0xFFFF).toShort())
        assertEquals(float.toByte(), (float.value.toInt() and 0xFF).toByte())
    }

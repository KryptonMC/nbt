...
class ExaminationTests {
    ...
    fun `examine arrays`() {
        ...
        assertEquals("[L;84L,983L,42L,93L]", StringTagExaminer().examine(LongArrayTag(longArrayOf(84, 983, 42, 93))))
    }
    ...
    @Test
    fun `examine list`() {
        val tags = listOf(StringTag.of("hello_world"), StringTag.of("goodbye_world"))
        assertEquals("[\"hello_world\",\"goodbye_world\"]", StringTagExaminer().examine(ListTag.immutable(tags, StringTag.ID)))
    }

    @Test
    fun `examine compound`() {
        val tags = mapOf("hello_world" to ByteTag.of(1), "goodbye_world" to LongTag.of(8))
        assertEquals("{goodbye_world:8L,hello_world:1b}", StringTagExaminer().examine(CompoundTag.immutable(tags)))
    }
}

...
class ReadTests {
    ...
    @Test
    fun `big test uncompressed`() {
        checkBigTest(TagIO.readNamed(getResource("bigtest.nbt")))
    }
    ...
    companion object {
        ...
        private fun getResource(name: String): InputStream =
            checkNotNull(Thread.currentThread().contextClassLoader.getResourceAsStream(name)) { "Could not find resource $name!" }
        ...
        private fun checkBigTest(input: Pair<String, Tag>) {
            ...
            val byteArrayTestName = "byteArrayTest (the first 1000 values of (n*n*255+n*7)%100, starting with n=0 (0, 62, 34, 16, 8, ...))"
            assertTrue(tag.contains(byteArrayTestName, ByteArrayTag.ID))
            assertTrue(tag.contains("listTest (long)", ListTag.ID))
            assertTrue(tag.getList("listTest (long)", LongTag.ID).data.isNotEmpty())
            ...
            assertTrue(tag.getList("listTest (compound)", CompoundTag.ID).data.isNotEmpty())

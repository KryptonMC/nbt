...
public interface NBTReader : Closeable {
    ...
    public fun read(): Tag = when (peekType().toInt()) {
        ...
        ListTag.ID -> {
            ...
            val list = ListTag.immutableBuilder(type)
            while (hasNext()) {
                list.add(read())
            }
            endList()
        ...
        CompoundTag.ID -> {
            ...
            val compound = CompoundTag.immutableBuilder()
            while (type != 0) {
                val name = nextName()
                val value = read()
    ...
    public companion object {
         ...
         *
         * @param input the binary source
         * @return a new binary NBT reader
         */
        @JvmStatic
        public fun binary(input: InputStream): NBTReader {
            val dataInput = if (input is DataInputStream) input else DataInputStream(input)
            return BinaryNBTReader(dataInput)
        }
    }
}

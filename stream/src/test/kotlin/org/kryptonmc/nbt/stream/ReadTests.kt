...
class ReadTests {
    ...
    fun bananrama() {
        ...
        val reader = BinaryNBTReader(DataInputStream(ByteArrayInputStream(output.toByteArray())))
        assertDoesNotThrow {
    ...
    fun `big test`() {
        ...
        assertEquals(0.49823147F.toBits(), reader.nextFloat().toBits())
        assertEquals("doubleTest", reader.nextName())
        assertEquals(0.4931287132182315.toBits(), reader.nextDouble().toBits())
